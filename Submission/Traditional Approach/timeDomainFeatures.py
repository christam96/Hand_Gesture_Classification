...
def AR(x, order, nsignals, nwin):
    # Had to use for loops as no library that handled 2D arrays was found
    AR_feat = np.empty([x.shape[1], order])
    for ii in np.arange(0, x.shape[1]):
        rho, sigma = yule_walker(x[:, ii], order=order)
        AR_feat[ii, :] = rho.reshape(1, -1)

    AR_feat = AR_feat.reshape(nwin, order*nsignals)
    return AR_feat
